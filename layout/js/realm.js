...
function addAnchorsInDescription(id, customPattern) {
    var
        regex = new RegExp('(https?:\\/\\/)?(www\\.)?(youtube\\.com|youtu\\.be|imgur\\.com|i\\.imgur\\.com|puu\\.sh|twitch\\.tv|reddit\\.com|redd\\.it|github\\.com|community\\.kabam\\.com|forums\\.wildshadow\\.com|realmeye\\.com|bluenosersguide\\.weebly\\.com|pfiffel\\.com)(\\/[\\w\\/\\.\\?=&;]+)?' + (customPattern ? '|' + customPattern : ''), 'gi'),
        div = $("#" + id);
    $('.description-line', div).each(function() {
        $(this).html($(this).html().replace(regex, function(match) {
            ...
            var extra = /realmeye\.com/.test(match) ? ' rel="nofollow"' : ' rel="nofollow external"';
